"""Find valid model names and try correct body formats."""
...
def mk_headers(accept="text/event-stream"):
    tid = uuid.uuid4().hex
    rid = str(uuid.uuid4())
    h = {
...
# ── 1. Get model list ────────────────────────────────────────────────────
print("=== Model List ===")
ml_url = "https://trae-api-cn.mchost.guru/api/ide/v1/model_list_by_function"
try:
    resp = httpx.post(ml_url, headers=mk_headers(accept=None), json={}, timeout=30.0)
...
# ── 2. Try llm_raw_chat with correct messages format ─────────────────────
print("\n\n=== llm_raw_chat with content array format ===")
raw_url = "https://trae-api-cn.mchost.guru/api/ide/v1/llm_raw_chat"
q = "Say hello in Chinese, one sentence only."

# Try various model names with correct messages format
for model in ["glm-5.2", "GLM-5.2", "gpt-4o", "claude-3.5-sonnet", "seed_m8", "auto"]:
    body = {
    ...
    print(f"\n--- model={model} ---")
    try:
        with httpx.stream("POST", raw_url, headers=mk_headers(), json=body, timeout=30.0) as resp:
            print(f"  Status: {resp.status_code}")
            n = 0
...
# ── 3. Try llm_utils_chat with different function values ─────────────────
print("\n\n=== llm_utils_chat with different function values ===")
chat_url = "https://trae-api-cn.mchost.guru/api/agent/v3/llm_utils_chat"

for func in ["chat", "completion", "code", "utils", "ask", "default"]:
    body = {
        "user_input": q,
        "model_name": "glm-5.2",
        "function": func,
    ...
    print(f"\n--- function={func} ---")
    try:
        with httpx.stream("POST", chat_url, headers=mk_headers(), json=body, timeout=20.0) as resp:
            print(f"  Status: {resp.status_code}")
            n = 0
            for line in resp.iter_lines():
                if line:

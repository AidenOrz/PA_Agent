"""TRAE Work CN - 用正确的 super_completion_query 请求体测试。
...
- user_query
- history_queries
- render_context: {variables: JSON.stringify({...})}
  - user_input_history
...
用法:
    python tools/_test_trae_super_v2.py
"""
...
def main() -> int:
    print("=" * 60)
    print("TRAE Work CN super_completion_query (完整请求体)")
    print("=" * 60)
    ...
    # 构造请求体(模拟 d 函数)
    variables = {
        "user_input_history": json.dumps(["用中文一句话回答: 1+1=?"], ensure_ascii=False),
    ...
    payload = {
        ...
        "target_language": "",
        "user_query": "用中文一句话回答: 1+1=?",
        "history_queries": "",
        "render_context": {"variables": json.dumps(variables, ensure_ascii=False)},
    }

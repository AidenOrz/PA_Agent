"""TRAE Work CN - user_query 需要 JSON 字符串格式。

之前发现:
- user_query="用中文..." → "invalid character 'ç'" (Go JSON 解析错误)
- user_query + function_type=recommend → "render_context.variables is required"

测试 user_query 传 JSON 字符串 + render_context.variables。

用法:
    python tools/_test_trae_super_v4.py
"""
...
def main() -> int:
    print("=" * 60)
    print("TRAE super_completion_query - user_query JSON 格式测试")
    print("=" * 60)
    ...
    # user_query 需要是 JSON 字符串
    # 尝试不同的 JSON 结构
    user_query_variants = [
        ("user_query=JSON字符串", json.dumps(q, ensure_ascii=False)),
        ("user_query=JSON对象{text}", json.dumps({"text": q}, ensure_ascii=False)),
        ("user_query=JSON对象{query}", json.dumps({"query": q}, ensure_ascii=False)),
        ("user_query=JSON对象{content}", json.dumps({"content": q}, ensure_ascii=False)),
        ("user_query=JSON数组[字符串]", json.dumps([q], ensure_ascii=False)),
        ("user_query=JSON对象{user_input}", json.dumps({"user_input": q}, ensure_ascii=False)),
        ("user_query=JSON对象{message}", json.dumps({"message": q}, ensure_ascii=False)),
        ("user_query=JSON对象{input}", json.dumps({"input": q}, ensure_ascii=False)),
        ("user_query=空JSON", json.dumps({}, ensure_ascii=False)),
    ]
    ...
    # 先测试每个 user_query 变体 (function_type=recommend + render_context)
    for label, uq in user_query_variants:
        payload = {
            "env_metadata": "{}",
        ...
        content = try_payload(url, headers, payload, f"recommend + {label}")
        if content:
            print(f"\n★★★ 成功! recommend + {label}")
            print(f"user_query JSON 格式: {uq}")
            return 0

    # 也测试 function_type=chat + user_query JSON
    for label, uq in user_query_variants[:5]:
        payload = {
            "function_type": "chat",
            "user_query": uq,
        }
        content = try_payload(url, headers, payload, f"chat + {label}")
        if content:
            print(f"\n★★★ 成功! chat + {label}")
            print(f"user_query JSON 格式: {uq}")
            return 0

"""Test Qoder CN response speed for different models."""
import json
import time
import uuid
import websocket
...
def test_model(model_key: str, question: str = "Reply with exactly: hi"):
    """Test a single model and return timing info."""
    print(f"\n{'='*60}")
...
results = []
for model_key in ["mmodel", "auto", "qmodel_38max"]:
    try:
        r = test_model(model_key)
        results.append(r)
    except Exception as e:
        print(f"  [ERROR] {e}")
        results.append({"model": model_key, "error": str(e), "total_time": 0})

# Summary
print(f"\n{'='*60}")
print("SUMMARY")
print(f"{'='*60}")
print(f"{'Model':<20} {'Total Time':<15} {'First Token':<15} {'Content Length':<15}")
print(f"{'-'*60}")
for r in results:
    if "error" in r:
        print(f"{r['model']:<20} ERROR: {r['error'][:40]}")
    else:
        print(f"{r['model']:<20} {r['total_time']:.2f}s{'':<8} {r['first_token_time']:.2f}s{'':<8} {len(r.get('content', ''))} chars")
